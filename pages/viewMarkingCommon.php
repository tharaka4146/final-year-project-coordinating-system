...
$sql = "SELECT count(distinct(kduEmail)) as countSend FROM $formName where indexNo != 1";
$result = $con->query($sql);
...
$sql = "SELECT count(indexNo) as countSend2 FROM $formName";
$result = $con->query($sql);

if ($result->num_rows > 0) {
    if ($row = $result->fetch_assoc()) {

        $countSend2 = $row['countSend2'];

        if ($countSend2 == 1) {
            $countSend = 0;
            $message = "no marking available yet";
        }
    }
}
...
<head>
    ...
    switch ($formName) {
        case "progressReview1":
            $topic = "Progress Review 1 Marking";
            break;
        case "progressReview2":
            $topic = "Progress Review 2 Marking";
            break;
        case "thesis":
            $topic = "Thesis Marking";
            break;
        case "paper":
            $topic = "Paper Submission Marking";
            break;
        case "final":
            $topic = "Final preseantation + Viva Marking";
            break;
        case "proposalEvaluation":
            $topic = "Proposal Evaluation Marking";
            break;
    }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800"><?php echo "$topic" ?></h1>

                    <h6 class="h4 mb-4 text-gray"><?php echo "$message" . ' '; ?></h6>
                    ...
                    <div class="row">
                        ...
                        for ($i = 0; $i < $countSend; ++$i) {
                            ...
                            if ($countSend == 1) {
                                $sql = "SELECT * from $formName where indexNo != 1 order by kduEmail desc limit $i1 OFFSET $i";
                                $result = $con->query($sql);
                            } else {
                                $sql = "SELECT * from $formName where indexNo != 1 order by kduEmail desc limit $i1 OFFSET $i1";
                                $result = $con->query($sql);
                            }
                            ...
                            $sql = "SELECT admissionNo from norm where kduEmail = '" . $kduEmail . "'";
                            $result = $con->query($sql);
                            ...
                            for ($j = 0; $j < 2; ++$j) {
                                ...
                                $sql = "SELECT (mark1 + mark2 + mark3 + mark4 + mark5 + mark6 + mark7 + mark8 + mark9 + mark10 + mark11 + mark12 + mark13 + mark14 + mark15 + mark16 + mark17 + mark18) as sum, supervisorEmail from $formName where kduEmail = '" . $kduEmail . "' order by kduEmail desc limit $j1 OFFSET $j ";
                                $result = $con->query($sql);
