...
$formName = 'progressReview2';

$sql = "SELECT count(*) as countSend FROM events where examiner1 = '" . $name . "' or examiner2 = '" . $name . "' ";
$result = $con->query($sql);
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800"><?php echo "$formName" . ' '; ?>presentation</h1>
                    ...
                    <div class="row">
                        ...
                        for ($i = 0; $i < $countSend; ++$i) {
                            ...
                            $sql = "SELECT * FROM events WHERE (examiner1 = '" . $name . "' OR examiner2 = '" . $name . "') order by start_event limit $i1 OFFSET $i";
                            $result = $con->query($sql);

                            if ($result->num_rows > 0) {
                                if ($row = $result->fetch_assoc()) {

                                    $admissionNo = $row['title'];
                                    $kduEmail = $row['name'];

                                    $firstName1 = $row['start_event'];
                                    $firstName = date('F d, Y', strtotime($firstName1));
                                    $lastName = date('h:i a', strtotime($firstName1));
                            ...
                            for ($j = 0; $j < 2; ++$j) {
                                ...
                                $sql = "SELECT (mark1 + mark2 + mark3 + mark4 + mark5 + mark6 + mark7 + mark8 + mark9 + mark10 + mark11 + mark12 + mark13 + mark14 + mark15 + mark16 + mark17 + mark18) as sum from progressReview2 where kduEmail = '" . $admissionNo . "' order by indexNo desc limit $j1 OFFSET $j ";
                                $result = $con->query($sql);
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="hoverbox rounded-soft text-center">
                                                ...
                                                <div style=" position: absolute; top: 55%; left: 55%; transform: translate(-50%, -50%); color:white;" class="centered"><i><?php echo $kduEmail; ?></i></div>
                                                <div style=" position: absolute; top: 55%; left: 60%; transform: translate(-50%, -50%); color:white;" class="centered">Marks : <b><?php echo $totSum; ?></b> %</div>

                                                <div class="hoverbox-content bg-dark p-5 align-items-center" style="text-align: center;">

                                                    <div style="margin: auto;">
                                                        <h6 class="leasd text-white"><?php echo "$firstName"; ?></h6>
                                                        <h6 class="leasd text-white"><?php echo "$lastName"; ?></h6>
                                                        <br>
                                                        <h6 class="leasd text-white"><?php echo $examiner1; ?></h6>
                                                        <h6 class="leasd text-white"><?php echo $examiner2; ?></h6>
                                                        <br>
                                                        <a class="btn btn-primary btn-md mt-1" href="progressReview2.php?kduEmail=<?php echo $admissionNo; ?>">Go to marking scheme</a>
                                                    </div>
