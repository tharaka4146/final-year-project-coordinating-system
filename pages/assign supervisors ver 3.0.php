...
<body onload="calculate(),colorChange()" data-spy="scroll" data-target="#navbarResponsive">
    ...
    <div class="form-row" style="width: 100%; margin-top: 00px; backgrsound-color: rgb(159, 207, 174); padding-top: 10px; padding-bottom: 10px;padding-left: 10px;padding-right: 10px;">
                        ...
                        ?>
                            <tr>
                                ...
                                <td style="width:150px;">
                                    ...
                                    echo '<a href="normView.php?kduEmail=' . $row["kduEmail"] . '" data-toggle="tooltip" data-placement="right" title=' . $row["firstName"] . '&nbsp;' . $row["lastName"] . ' >' . $row["admissionNo"] . '</a>' . '<br>';
                                    ?>
...
-->
                                ...
                                $supervisorListOrig = array("supervisor1", "supervisor2", "supervisor3");
                                $supervisorList = array("s1", "s2", "s3");
                                    ...
                                    <td>
                                        ...
                                        <select class="custom-select custom-select-sm" onclick="calculate(), colorChange('<?php echo $admissionNo ?>','<?php echo $b ?>')" id="<?php echo $admissionNo . $b; ?>" style="width: 100%; color:#232b2b; background-color:	#f2f1e7">
                                            ...
                                            <optgroup value="0" label="requested">
                                                <option value="<?php echo $row["$supervisorListOrig[$b]"]; ?>" hidden>
                                                    <?php echo $row["$supervisorListOrig[$b]"]; ?></option>
                                                <option value="<?php echo $row["$supervisorListOrig[$b]"]; ?>" style="background-color:lightgreen;">
                                                    <?php echo $row["$supervisorListOrig[$b]"]; ?></option>
                                            </optgroup>

                                            <optgroup value="3" label="not assigned">
                                                <option value="not assigned" style="background-color:pink;">Not assigned</option>
                                            </optgroup>
                                            ...
                                            <optgroup value="2" label="other research area">
                                                ...
                                                $sql = "SELECT count(distinct(name)) as supervisorCountArea FROM supervisors where area not in('$areas2') and name not in('$sameArea')";
                                                $result = $conn->query($sql);
                                                ...
                                                for ($d = 0; $d < $supervisorCountArea; $d++) {
                                                    ...
                                                    $sql = "SELECT distinct(name) FROM supervisors where area not in('$areas2')  and name not in('$sameArea') order by name limit $d1 OFFSET $d";
                                                    $result = $conn->query($sql);
...
*/
                ...
                function calculate() {
                    ...
                    ?>
                            ...
                            */
                            ...
                            xmlhttp.open("GET", "updateSupervisor.php?q=" + a + "&b=" + b + "&c=" + c);
                            xmlhttp.send();
         ...
         */
                    ...
                    var result1 = {};
                    oneArray.forEach(function(x) {
                        result1[x] = (result1[x] || 0) + 1;
                    });

                    var result2 = {};
                    twoArray.forEach(function(x) {
                        result2[x] = (result2[x] || 0) + 1;
                    });

                    var result3 = {};
                    threeArray.forEach(function(x) {
                        result3[x] = (result3[x] || 0) + 1;
                    });
